...
class BlackBoxSampler(AbstractSampler):
    def __init__(
        ...
        **kwargs
        ...
        self.debug_env = self.train_envs
    ...
    def get_envs(self, env_type: str = "training"):
        if env_type == "training":
            self.train_envs.seed(self.seed)
        elif env_type == "testing":
            self.train_envs.seed(self.seed + 10000)
        return self.train_envs
    ...
    def run(
        ...
        for ep_idx in range(ep_per_env):
            ...
            # Initial conditions
            episode_init_state = to_ts(episode_init_state, self.dtype, self.device)

            # Policy prediction
            episode_params_mean, episode_params_L = policy.policy(episode_init_state)

            assert_shape(episode_params_mean, [num_env, dim_mp_params])
            assert_shape(episode_params_L, [num_env, dim_mp_params, dim_mp_params])
            list_episode_params_mean.append(episode_params_mean)
            list_episode_params_L.append(episode_params_L)

            episode_action = policy.sample(
                require_grad=False,
            ...
            episode_log_prob = policy.log_prob(
                episode_action,
                params_mean=episode_params_mean,
                params_L=episode_params_L,
            )
            ...
            episode_init_state, episode_reward, episode_done, episode_info = envs.step(
                to_np(episode_action)
            )

            # Episode rewards
            assert_shape(episode_reward, [num_env])
            episode_reward = to_ts(np.asarray(episode_reward), self.dtype, self.device)
            list_episode_reward.append(episode_reward)

            # Episode dones
            episode_done = to_ts(np.asarray(episode_done), torch.bool, self.device)
            assert_shape(episode_done, [num_env])
            list_episode_done.append(episode_done)

            # Update training steps
            episode_length = util.get_item_from_dicts(episode_info, "trajectory_length")
            ...
            if self.task_specified_metrics:
                for metric in self.task_specified_metrics:
                    metric_value = util.get_item_from_dicts(
                        episode_info, metric, lambda x: x[-1] # TODO take other than last one?
                    )

                    metric_value = to_ts(metric_value, self.dtype, self.device)
        ...
        results["segment_params_mean"] = torch.cat(list_episode_params_mean, dim=0)
